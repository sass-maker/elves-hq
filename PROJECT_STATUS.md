...
**IN scope for V0:** manual local product/folder registry, bounded local folder browser, local product removal, editable product settings, terminal drawer dashboard with captured Codex/run output, dashboard terminal creation, resizable terminal drawer grid, focused terminal drawer mode, terminal-level instruction input, terminal-level run controls, terminal-level ask answering, terminal-level decision actions, product-first Fleet Pulse, compact overview switcher, live/stale local sync visibility, room signal filters and sorting, project-wise task backlog with lightweight task lifecycle, task rooms, active/all room deck filtering, slideable room deck, organized room output shelf, elf assignment/status, room asks, logs, artifacts, room activity timeline, captured run prompts, next-run instruction composer, expanded editable local product memory, built-in playbooks, decision actions, neutral room close action, fix-request retry context, runtime/retry budget guards, approved diff apply, local daily brief with snapshots, local Elf FM text stations, selectable check gates, CodeVetter review gate adapter, generated worktree cleanup controls, draggable resizable room UI, tasteful elf workbench animation, local-only operation.
...
- Local daemon exposes `GET /api/health`, `GET /api/workspace`, `GET /api/needs-me`, `GET /api/briefs/daily`, `GET /api/briefs/daily.md`, `POST /api/briefs/daily/save`, `GET /api/briefs/daily/latest-snapshot`, `GET /api/fm/feed`, `GET /api/folders`, `POST /api/products`, `POST /api/products/inspect-path`, `POST /api/products/:id/settings`, `POST /api/products/:id/remove`, `GET /api/products/:id/inspection`, `GET /api/products/:id/memory`, `POST /api/products/:id/memory/:section`, `POST /api/tasks`, `POST /api/tasks/:id/status`, `POST /api/tasks/:id/assign-room`, `POST /api/rooms`, `POST /api/rooms/:id/notes`, `POST /api/rooms/:id/decision`, `POST /api/rooms/:id/asks/:askId/answer`, `GET /api/rooms/:id/runs`, `POST /api/rooms/:id/runs/start`, `POST /api/rooms/:id/transcript`, `GET /api/rooms/:id/transcript`, `GET /api/runs/:id/prompt`, `GET /api/runs/:id/logs`, `POST /api/runs/:id/apply-diff`, and `POST /api/runs/:id/kill`.
...
- Room runs support local dry runs and read-only Codex inspection, stream stdout/stderr into room logs, persist run records, capture durable per-run logs, support kill, and enforce a default runtime cap.
- Codex read and worktree modes preflight the configured Codex command before creating run records; set `ELVES_HQ_CODEX_COMMAND` when the binary is not named `codex`.
- Codex JSONL stdout is normalized into founder-readable room log lines while the durable per-run log still captures the raw stdout/stderr stream.
- Selected-room run history shows elapsed duration with running/exit state from persisted run timestamps.
...
- Room-aware run prompts are captured to `runs/<run-id>/prompt.md`, stdout/stderr is captured to `runs/<run-id>/logs.txt`, and prompts include acceptance criteria, founder notes, prior decisions, artifacts, and recent logs.
- Rooms include a next-run instruction composer for tactical one-off guidance; instructions are embedded in the generated prompt for Dry, Read, Draft, and Build runs, then cleared after a successful launch.
...
- Room deck items are simplified selector rows focused on title, status, product, elf, last activity, and high-signal ask/artifact count; detailed summaries and evidence stay in the selected room detail.
...
- Product memory is stored as ignored local Markdown under `memory/<product-slug>/`, editable in the room cockpit, injected into generated run prompts, and covers Product, Strategy, Customers, Roadmap, Architecture, Decisions, Do Not Do, Recent Learnings, Feedback, Metrics, and Brand sections.
...
- Worktree-backed runs create isolated branches/worktrees under ignored `runs/<run-id>/worktree`, persist the generated workspace path and branch name on the run record, capture `git diff` patches to `runs/<run-id>/diff.patch`, and attach ready diff artifacts to the room.
- Write-capable Codex mode is available only inside isolated worktrees; it is not used for direct repo writes.
- Check gates run inside the isolated worktree, can use Auto or a selected detected package script (`check`, `typecheck`, `test`, or `build`), capture output to `runs/<run-id>/check.log`, attach pass/fail test artifacts to the room, and mark rooms failed when the current check gate fails.
...
- Room action panels keep a command-first default view and collapse run instructions plus approval-gate details behind summary rows, while still showing whether Check and CodeVetter are waiting, required, passed, or failed before approving a diff.
...
- The cockpit opens to a terminal command-center start screen with one room-backed terminal drawer per active task in the selected project scope, a visually distinct intervention pane for Needs Me signals, captured latest run/Codex stdout-stderr where available, and a direct path into the full room workbench.
- The command-center frames active task rooms as Codex terminal drawers, with snapped drag resizing, local drawer-size persistence, focus mode for expanding one drawer into a large single terminal, a compact drawer rail for switching, and far-view state strips where green means working, amber means needs input, red means failed/stuck, blue means ready, and gray means idle.
- The command-center can create a new Codex terminal drawer directly from the dashboard using existing project, elf, playbook, and acceptance-criteria inputs without opening the older room creation pane.
- Command-center terminal panes can start read-only Codex, worktree Codex, and dry-run modes directly, then switch to a stop control while that terminal has an active run.
- Command-center terminal panes show daemon run-start errors inline and refresh room state so preflight failures are visible instead of silent.
- Command-center terminal panes include a `$` instruction line; read/build/dry sends that terminal draft through the existing captured run prompt path.
- Command-center terminal panes show active elf asks above captured run output, replace run controls with ask options while unresolved, and submit the `$` line as optional answer context.
- Command-center terminal drawers expose compact decision actions for ready, failed, and blocked decisions using the existing decision endpoint, while keeping ask-answer options and active stop controls higher priority.
...
- Daily Brief snapshots can be saved to ignored local Markdown under `runs/daily-briefs/` and reopened from the cockpit as the latest saved operating record.
...
- Founder can close stale or no-longer-relevant rooms with a neutral low-risk decision that clears asks, logs the action, marks the room done, and removes it from the active room deck without implying approval or rejection.

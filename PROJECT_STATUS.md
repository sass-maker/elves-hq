...
- 2026-07-07 - Task-room creation, explicit elf assignment, isolated worktree runs, diff capture, captured prompts, editable local product memory, built-in playbooks, structured elf asks, room transcripts, fix-request retry context, local Daily Brief, check gates, CodeVetter gate adapter, generated worktree cleanup, a signal-backed Needs Me queue, founder decision actions, and blocking-gate approval guards landed in the local cockpit.
